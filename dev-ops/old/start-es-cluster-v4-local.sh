...
CLUSTER_DIR=/data/local_drive/elasticsearch/${CLUSTER_NAME}
DATA_DIR=/data/elasticsearch/${CLUSTER_NAME}/data
#PATH_CONFIGS=${CLUSTER_DIR}/configs
PATH_CONFIGS=$CLUSTER_DIR/configs
CONFIGS_DIR=/etc/elastic-stack
...
ES_JAVA_OPTS="-Xms4g -Xmx4g" ES_PATH_CONF="${PATH_CONFIGS_ES}/${NODE_NAME}" ./bin/elasticsearch  -E path.data=${DATA_DIR}/${NODE_NAME} -p ${PIDS_DIR}/${NODE_NAME}.pid -E path.logs=${LOGS_DIR}/${NODE_NAME} -E "node.attr.my_node_type=hot"   -E "node.name=node-0-hot" -E "node.master=false" -E "cluster.name=${CLUSTER_NAME}" -d
...
#ES_JAVA_OPTS="-Xms2g -Xmx2g" ES_PATH_CONF="${PATH_CONFIGS_ES}/${NODE_NAME}" ./bin/elasticsearch -E path.data=${DATA_DIR}/${NODE_NAME} -p ${PIDS_DIR}/${NODE_NAME}.pid -E path.logs=${LOGS_DIR}/${NODE_NAME} -E "node.attr.my_node_type=warm"   -E "node.name=node-1-warm" -E "node.master=false"  -E "cluster.name=${CLUSTER_NAME}" -d
echo "Started Elasticsearch $NODE_NAME"
...
#ES_JAVA_OPTS="-Xms2g -Xmx2g" ES_PATH_CONF="${PATH_CONFIGS_ES}/${NODE_NAME}" ./bin/elasticsearch -E path.data=${DATA_DIR}/node2 -p ${PIDS_DIR}/${NODE_NAME}.pid -E path.logs=${LOGS_DIR}/${NODE_NAME} -E "node.attr.my_node_type=cool"   -E "node.name=node-2-cool" -E "node.master=true" -E "cluster.name=${CLUSTER_NAME}" -d
echo "Started Elasticsearch $NODE_NAME" 
