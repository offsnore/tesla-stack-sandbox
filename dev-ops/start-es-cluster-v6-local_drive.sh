...
sudo chown -R andrew:wheel ${PATH_CONFIGS}/${NODE_NAME}
sudo chown -R andrew:wheel ${DATA_DIR}/${NODE_NAME}
sudo chown -R andrew:wheel ${LOGS_DIR}/$NODE_NAME
...
if [ ! -z $PATH_CONFIGS/$NODE_NAME  ]; then
  ...
  echo "Copying default configs from: $DEFAULT_CONFIGS_PATH_ES/$NODE_TYPE"
  if [ ! -f $PATH_CONFIGS/$NODE_NAME/elasticsearch.yml ]; then 
    sudo cp -rf $DEFAULT_CONFIGS_PATH_ES/$DEFAULT_NODE_TYPE/$NODE_TYPE $PATH_CONFIGS/$NODE_NAME
  fi 
fi
if [ -z ES_HOME ]; then 
  cd $ES_HOME
