...
ES_JAVA_OPTS="-Xms2g -Xmx2g" ./bin/elasticsearch -E path.data=${DATA_DRIVE}i/node1 -p /data/data_drive/pid/9211 -E path.logs=/var/log/elasticsearch/data_drive-node1 -E "node.attr.my_node_type=warm"   -E "node.name=node-1-warm" -E "node.master=false"  -E "cluster.name=data-drive-cluster" -d
